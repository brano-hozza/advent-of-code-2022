with open('input.txt') as f:
    ...
    for pair in pairs:
        ...
        # check if first range overlaps with second range
        if int(firstRange[0]) <= int(secondRange[0]) and int(firstRange[1]) >= int(secondRange[0]):
            total += 1
            continue

        # check if second range overlaps with first range
        if int(secondRange[0]) <= int(firstRange[0]) and int(secondRange[1]) >= int(firstRange[0]):
            total += 1
            continue
